...
const runSimulation = (inputs) => {
  ...
  const timeValue = Math.max(0, hoursSaved * (newPrice - variableCost));
...
const buildMessage = (type, ctx) => {
  const templates = {
    ...
    official: `Szanowny/a ${ctx.clientName},\n\nInformuję o waloryzacji cennika od ${ctx.startDate}. Nowa cena za ${ctx.packageName} wyniesie ${ctx.newPrice} zł.\n\nDla obecnych klientów obowiązuje okres przejściowy do ${ctx.graceDate}.\n\nZ poważaniem,`,
